...
class GripperWSG32(GripperBase):
    ...
    The URDF file contains concave shapes of the moving links. As they are loaded into pybullet, the collision shape
    will actually be a convex approximation of that shape - this changes the collision behaviour and prevents the
    fingers from closing properly.
    Therefore, we load the gripper with self-collisions disabled and perform position control for both fingers
    independently. This does not accurately simulate the true behaviour, but should be close enough.
    (Alternative would be to better model the shapes, or get a more accurate VHACD.)
    """
    ...
    def close(self):
        # velocity control with follower does not work properly
        variant = 'position_control'
        if variant == 'velocity_with_follower':
            self._bullet_client.setJointMotorControl2(
                self.body_id,
        ...
        if variant == 'position_control':
            self._bullet_client.setJointMotorControlArray(
                self.body_id,
                self._moving_joint_ids,
                self._bullet_client.POSITION_CONTROL,
                targetPositions=[0]*2,
